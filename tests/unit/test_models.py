...
class TestAltTextResult:
    ...
    def test_alt_text_too_long(self):
        """Test that alt-text > 250 chars is truncated (no longer raises)."""
        # Alt-text is now truncated instead of rejected
        result = AltTextResult(
            image_id="img-001",
            alt_text="x" * 251,  # Too long, but will be accepted
            confidence_score=0.95,
            validation_passed=False,  # Validation fails for too-long text
            tokens_used=100,
            processing_time_seconds=1.5,
        )
        # Should still create the result even with long text
