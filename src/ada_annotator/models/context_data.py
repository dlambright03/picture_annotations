...
class ContextData(BaseModel):
    ...
    external_context: str | None = Field(
        None, description="Context from external file (highest priority)"
    )
    document_context: str = Field(
        ..., description="Document-level metadata (title, subject)"
    )
    section_context: str | None = Field(
        None, description="Section or heading context"
    )
    page_context: str | None = Field(
        None, description="Page or slide context (PPTX only)"
    )
