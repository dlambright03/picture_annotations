...
class ImageMetadata(BaseModel):
    ...
    page_number: Optional[int] = Field(
        None,
        gt=0,
        description="Page or slide number (1-indexed)"
    )
    position: Dict = Field(
        default_factory=dict,
        description="Position metadata (format-specific)"
    )
    existing_alt_text: Optional[str] = Field(
        None,
        description="Pre-existing alt-text if present"
    )
